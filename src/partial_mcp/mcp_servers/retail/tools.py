...
-----------------------------
This is a modified version of `tau2`'s `src/tau2/domains/retail/tools.py`.
"""
...
class RetailTools(BaseModel):  # Tools
    ...
    def __init__(self, db: RetailDB) -> None:
        ...
        server.tool(self.calculate)
        server.tool(self.cancel_pending_order)
        server.tool(self.exchange_delivered_order_items)
        server.tool(self.find_user_id_by_name_zip)
        server.tool(self.find_user_id_by_email)
        server.tool(self.get_order_details)
        server.tool(self.get_product_details)
        server.tool(self.get_user_details)
        server.tool(self.list_all_product_types)
        server.tool(self.modify_pending_order_address)
        server.tool(self.modify_pending_order_items)
        server.tool(self.modify_pending_order_payment)
        server.tool(self.modify_user_address)
